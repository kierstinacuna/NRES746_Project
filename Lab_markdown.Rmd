...
So far, we've scaled and transferred our data, and used a variable selection tool to narrow down our predictor variables. Say we want to test the hypothesis that our selected predictor variables are impacting the fish community in the Doubs river. There are functions that allow us to obatin R^2^ and p-values from an RDA!  
...
In this last exercise, we will run an RDA on our selected variables, plot the output, and assess its R-squared value and significance. After completing these tasks, answer the following questions in your word document:
