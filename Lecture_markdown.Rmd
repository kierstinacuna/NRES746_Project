...
If we use the Kaiser-Guttman Criterion, we can how much how much variation is captured by the 5 selected principal components.
...
With NMDS, the relationship among objects is the priority over showing the distances between object. Additionally, the almost all of the variation among sites can be visualized with less than three axes. Therefore, the biplot produced by an NMDS is a better two dimensional representation of similarity between objects than PCA, CA, and PCoA. 
...
Unlike Unconstrained Ordination, Constrained Ordination can by used to test hypothesis about the relationship between the environmental variables and species composition. It can partition the variance of species composition into the fraction explained by environmental variables (associated with constrained ordination axes), and not explained by environmental variables (unconstrained ordiation axes).   
...
The Redundancy Analysis models the effect of an explanatory matrix on a **response matrix**. That idea is important, as we have usually modeled multiple explanatory variables in one variable. A summary of the technique is that an ordination is done on a response matrix **Y** to obtain ordination axes that are linear combinations of explanatory matrix **X**. 

Like PCA, explanatory variables can be quantitative (centered, standardized, transformed or normalized, and collinearity reduced), qualitative, and binary variables. 

### How RDA Works

1. Multiple regression, where each object in **Y** is regressed on the explanatory variables **X**, which produces a matrix of fitted values **Y**~***fit***~

![](images/Lab_1.png){Width="60%"}

2. Run a PCA on the matrix of fitted values **Y**~***fit***~, which produces eigenvalues and eigenvectors, which for an RDA are called canonical axes. These axes are stred in a matrix ***Z***, which correspond to the linear combinations of the explanatory variables in ***X***. In analyzing community composition, these canonical axes are interpreted as complex environmental gradients. 
