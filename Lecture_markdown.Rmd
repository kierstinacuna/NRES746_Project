...
The Redundancy Analysis models the effect of an explanatory matrix on a **response matrix**. This is an important distinction from the models we've made so far, which have only had one response variable. A summary of the technique is that an ordination is done on a response matrix **Y** to obtain ordination axes that are linear combinations of explanatory matrix **X**. 
